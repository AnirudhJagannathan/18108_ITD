...
public class TeleOpTest extends OpMode {
    ...
    TeleOpB.SlideState slideState = TeleOpB.SlideState.START;
        ...
        switch (slideState) {
            case START:
                if (gamepad1.y || Range.clip(gamepad2.right_stick_y, -1, 1) < -0.25) {
                    slidePos = SLIDE_HIGH;
                    slideState = TeleOpB.SlideState.EXTEND;
                    down = false;
                ...
                if (Math.abs(slides.getVSlidesPos() - slidePos) < 75) {
                    ...
                    if (slidePos == SLIDE_HIGH) {
                        if (Range.clip(gamepad2.right_stick_y, -1, 1) > 0.25) {
                            slidePos = SLIDE_LOW;
                            slideState = TeleOpB.SlideState.RETRACT;
                        }
                        ...
                        if (gamepad1.x) {
                            ...
                            slideState = TeleOpB.SlideState.SPEC;
                        }
                        if (Range.clip(gamepad2.right_stick_y, -1, 1) < -0.25)
                            slidePos = SLIDE_HIGH;
                        if (Range.clip(gamepad2.right_stick_y, -1, 1) > 0.25) {
            ...
            case RETRACT:
                if (Math.abs(slides.getVSlidesPos() - SLIDE_LOW) < 150) {
                    slideState = TeleOpB.SlideState.START;
                }
                break;
            default:
                slideState = TeleOpB.SlideState.START;
